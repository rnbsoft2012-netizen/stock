...
추가할 때마다 세션을 새로 만들어야 하는 것은 아니다. 다만 트리거 생성 도구에
커넥터를 첨부하는 것은 여전히 조직 정책으로 막혀 있다.

### 메일 본문 만들기

`scripts/mailbody.py`가 만든다. 카톡용으로 이미 쓴 뉴스·전략 문장을 그대로
재사용하므로, 두 경로가 다른 말을 하지 않는다.

```bash
python scripts/brief.py > /tmp/brief.json
# notes.json: {"<티커>": {"news": "...", "strategy": "..."}, ...} — 카톡에 쓴 문장 그대로
python scripts/mailbody.py /tmp/notes.json /tmp/brief.json
```

**두 번째 인자(브리핑 JSON)를 반드시 넘긴다.** 생략하면 시세를 새로 조회하는데,
장이 열린 뒤에 실행하면 카톡으로 이미 보낸 숫자와 어긋나고 주목 종목 5개도
달라진다. 같은 아침의 카톡과 메일은 같은 실행 결과를 써야 한다.

출력의 `subject`/`text`/`html`을 그대로 `create_draft`에 넣는다
(`to`: rnbsoft2012@gmail.com, `body`: text, `htmlBody`: html).

메일 본문은 길이 제한이 없으므로 카톡과 달리 다음을 모두 담는다:
- 상단: 요약 (카톡 [1/6]과 동일)
- 중단: 주목 종목 5개의 수치 + 뉴스 + 전략 (카톡과 동일하되 줄임 없이)
- 하단: **전 종목 표** (23종목, 카톡에는 넣지 않는 부분)
