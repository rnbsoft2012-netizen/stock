...
6. PlayMCP 카카오톡 나에게 보내기(`KakaotalkChat-MemoChat`)로 [1/6]부터
   순서대로 6건 전송한다.
7. `scripts/report.py`로 전 종목 카드 리포트를 만들어 rnbsoft2012@gmail.com 앞으로
...
3. list_triggers로 남은 트리거를 확인하고, 이 새 세션에 묶이지 않은 것이
   있으면 delete_trigger로 정리해 주세요. 이전 세션의 매일 07:00 트리거는
   2026-08-06에 이미 삭제했지만, 그때 서버가 끊겨 정리하지 못한 일회성
   재시도 트리거가 남아 있을 수 있습니다.
```

**주의**: 이전 세션의 매일 07:00 트리거는 2026-08-06 정리 시점에 삭제됐다.
따라서 **새 세션에서 트리거를 만들기 전까지는 아침 브리핑이 나가지 않는다.**
